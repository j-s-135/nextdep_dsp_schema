...
from .DataTypeApiProvider import DataTypeApiProvider
from .ContentDefinition import ContentDefinition
from .DictionaryApiProviderWrapper import DictionaryApiProviderWrapper
...
class SchemaDefBuild(object):
    ...
    def __createDocumentDict(self, schemaGroupName, documentDefHelper):
        ...
        if dH:
            ...
            for cd in cdL:
                ...
                rD["COLLECTION_SUB_CATEGORY_AGGREGATES"][cN] = None
        #
        return rD
    ...
    def __createJsonLikeSchema(
        ...
        mandatorySubcategoryAttributes = False
        ...
        for catName, fullAtNameList in dictSchema.items():
            ...
            #
            # Not needed for DSP
            # if not isUnitCard:  # Don't overwrite if already True 
        ...
        if dataTypingU == "JSON":
            ...
            schemaRepo = "https://github.com/wwpdb/nextdep_dsp_schema/"
            desc1 = "PDBx/mmCIF JSON schema derived from the %s content type schema. " % schemaGroupName
